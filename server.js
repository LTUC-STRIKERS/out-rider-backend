...
server.post('/newfavorite',addNewfav);

async function addNewfav (req,res){
  ...
  const newPlace = new favResturantModel({
    name:obj.name,
    rating:obj.rating,
  ...
  favResturantModel.find({ email: usermail }, function (err, userBook) {
    if (err) {
